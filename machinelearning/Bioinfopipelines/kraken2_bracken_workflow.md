...
Notes
Replace /path/to/kraken2_db with the actual path to your Kraken2 database.
Make sure to adjust the read length (-r) in Bracken if using reads longer or shorter than 150 bp.
If working with paired-end reads, adapt the Kraken2 command accordingly.
Use adequate disk space and memory—classified FASTQ files may be several GB in size.
This workflow was tested with real metagenomic data using Kraken2 and Bracken, and is suitable for reproducible documentation or GitHub submission.
